...
_Last updated: 2026-07-15 (One-Stop brief Phase 2: executable spine shipped)_
...
- [ ] **Phase 3 — depth pass on the 10 projects** — next up. 3–4 steps grows to 6–10 with the fixed step anatomy; every Verify step maps to a Phase 2 lab; adds the `ship-it` T3 project; resolves the `estimatedTime` backlog item. Note: the project count goes 10 to 11 there, so CLAUDE.md's "9 real programs" line gets fixed in that session.
